...
/**
 * This is the model class for table "all_mon_ttcc_results".
 *
 * The followings are the available columns in table 'all_mon_ttcc_results':
 * @property integer $SLNO
 * @property string  $RINO3
 * @property string  $RISO3
 * @property string  $Commodity_Code
 */
class DbAllMonTtccResults extends ActiveRecordTradeData {
    /**
     * @return string the associated database table name
     */
    public function tableName () {
        return 'ALL_MON_TTCC_RESULTS';
    }
    ...
    public function rules () {
        ...
        return array(
            array('RINO3', 'required'),
            array('RINO3, RISO3', 'length', 'max' => 3),
            array('Commodity_Code', 'length', 'max' => 4),
            // The following rule is used by search().
            array('SLNO, RINO3, RISO3, Commodity_Code', 'safe', 'on' => 'search'),
        );
    }
    ...
    public function search ($options = null) {
        ...
        $criteria->compare('RINO3', $this->RINO3, true);
        $criteria->compare('RISO3', $this->RISO3, true);
        $criteria->compare('Commodity_Code', $this->Commodity_Code, true);
     ...
     * @return AllMonTtccResults the static model class
     */
    ...
    public function sampleData () {
        ...
        foreach ($reporters as $reporter) {
            foreach($commodities as $commodity) {
                $exists = DbAllMonTtccResults::model()->findByAttributes(array(
                ...
                if (empty($exists)) {
                    ...
                    $model->Commodity_Code = $commodity;
                    for ($y = 2010; $y < 2020; $y++) {
                        for ($m = 1; $m <= 12; $m++) {
                            $month = ($m < 10 ? '0' . $m : $m);
