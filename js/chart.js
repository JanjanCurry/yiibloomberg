...
$(function () {
    ...
    $(document).on('click', '.download-btn', function () {
        ...
        if (typeof chartId !== 'undefined') {
            renderPrintableChart(chartId, 'image');
        }
    });
    ...
    $(document).on('click', '.chart-group-png', function () {
        ...
        $(this).parents('.chart-group').find('.chart-item').each(function () {
            ...
            renderPrintableChart(chartId);
        });
        renderPrintableChart(chartId, 'imageMulti');
    });
    ...
    $(document).on('click', '.pdf-btn', function () {
        var chartId = $(this).parents('.chart-item').data('chart-id');
        if (typeof chartId !== 'undefined') {
            renderPrintableChart(chartId, 'pdf');
        }
    });

    $(document).on('click', '.chart-group-pdf', function () {
        var chartIds = [],
            length = $(this).parents('.chart-group').find('.chart-item').length;
        $(this).parents('.chart-group').find('.chart-item').each(function (i) {
            var chartId = $(this).data('chart-id')
            chartIds.push(chartId);
            renderPrintableChart(chartId, ((i + 1) === length ? 'pdfMulti' : null));
        });
    });
    ...
    $(document).on('click', '.mom-row-toggle-btn', function () {
        var open = $(this).data('open'),
            group = $(this).parents('.mom-row').data('group');
        ...
        $(this).parents('table').find('.mom-row').each(function () {
            console.log($(this).data('group'), group, $(this).data('group') === group);
            if (!$(this).hasClass('mom-row-toggle') && $(this).data('group') === group) {
                if (open === 1) {
    ...
    $(document).on('click', '.correlation-row-toggle-btn', function () {
        var open = $(this).data('open');

        if (open === 1) {
            $(this).data('open', 0);
            $(this).find('.fa').addClass('fa-plus').removeClass('fa-minus');
            $(this).parents('tr').find('.correlation-row-info').hide();
        } else {
            $(this).data('open', 1);
            $(this).find('.fa').addClass('fa-minus').removeClass('fa-plus');
            $(this).parents('tr').find('.correlation-row-info').show();
        }
    });
});
...
function drawChart (chartId, showLoading) {
    ...
    data = data.data();
    if ($('#ctmStartDate').length > 0) {
        data.startTime = $('#ctmStartDate').val();
    }
    if ($('#ctmEndDate').length > 0) {
        ...
        success: function (returnData) {
            if (returnData.valid) {
                ...
                if (!group.find('.chart-item[data-chart-id=' + chartId + ']').length) {
                    ...
                    var html = '<div class="chart-item ' + returnData.chart.col + '" data-chart-id="' + chartId + '"><div class="chart-item-header-container" data-chart-id="' + chartId + '"></div><div class="chart-item-chart-container"><div class="chart-item-chart" id="' + chartId + '"></div></div><p class="chart-item-source"></p><p class="chart-notes"></p></div>',
                        written = false;
...
function renderChart (chart) {
    ...
    if (typeof chart.html !== 'undefined') {
        group.find('#' + chart.chartId).html(chart.html);
        return false;
    }

    //add non-chart data
    var $table = group.find('.chart-item-table[data-chart-id=' + chart.chartId + ']');
    $table.html(chart.table);
    $table.find('.mom-row').each(function () {
        if ($(this).hasClass('mom-row-toggle')) {
            $(this).prepend('<a href="#" class="mom-row-toggle-btn" data-open="0"><i class="fa fa-plus"></i></a> ');
        } else {
            $(this).parents('tr').hide();
        }
    });
    $table.find('.correlation-row').each(function () {
        $(this).prepend('<a href="#" class="correlation-row-toggle-btn" data-open="0"><i class="fa fa-plus"></i></a> ');
        $(this).parents('tr').find('.correlation-row-info').hide();
    });
    tableFixedHeader();
    ...
    if (typeof chart.source !== 'undefined' && chart.source.length > 0) {
        group.find('.chart-item[data-chart-id=' + chart.chartId + '] .chart-item-source').text('Source(s): ' + chart.source.join(', ')).show();
        data.data('source', chart.source.join(', '));
    }
    ...
    if (typeof chart.notes !== 'undefined' && !$.isEmptyObject(chart.notes)) {
        ...
        data.data('notes', chart.notes).show();
    } else {
        data.data('notes', '');
    }
    ...
    switch (chart.chartType) {
        ...
        case 'bar':
            var chartObj = new google.visualization.BarChart(document.getElementById(chart.chartId));
            var chartObjPrint = new google.visualization.BarChart(document.getElementById(chart.chartId + '-print'));
            break;
...
function renderChartLoading (group) {
    if (group.find('.chart-loading').length === 0) {
        group.append('<div class="chart-loading" data-count="1"><div class="chart-loading-text"><span class="fa fa-spinner fa-spin"></span><br /><br />Please wait a moment while we generate your real-time report</div></div>');
...
function renderPrintableChart (chartId, downloadType, shareImage) {
    downloadType = (typeof downloadType !== 'undefined' ? downloadType : null);
    ...
    if (chart.printOptions.backgroundColor !== bgColor) {
        chart.drawPrint = false;
    }
    chart.printOptions.backgroundColor = bgColor;
    chart.viewPrintOptions.backgroundColor = bgColor;

    if (!chart.drawPrint) {
        var event = google.visualization.events.addListener(chart.chartObjPrint, 'ready', function () {
            chartsLog[chartId].drawPrint = true;
            google.visualization.events.removeListener(event);
            renderPrintableChart(chartId, downloadType, shareImage);
        });
        chart.chartObjPrint.draw(chart.chartData, chart.printOptions);
        return false;
    }
    ...
    var event = google.visualization.events.addListener(chart.chartObjPrint, 'ready', function () {
        ...
        html2canvas(newLegend, {
            onrendered: function (canvas) {
                ...
                switch (downloadType) {
                    case 'image':
                        downloadChartImage(chartId, false, shareImage);
                        break;

                    case 'imageMulti':
                        downloadChartImage(chartId, true, shareImage);
                        break;

                    case 'pdf':
                        downloadPDF(chartId, false)
                        break;

                    case 'pdfMulti':
                        downloadPDF(chartId, true)
                        break;
        ...
        modal.find('.chart-search-list').each(function () {
            if (typeof $(this).data('select-disable') !== 'undefined') {
                ...
                for (var key in restrict) {
                    if (restrict[key].indexOf(",") >= 0) {
                        var subRestrict = restrict[key].split(',');
                        for (var key2 in subRestrict) {
                            modal.find('.chart-search-list .list-group-item[data-ref=' + subRestrict[key2] + ']').addClass('list-group-item-disabled list-group-item-selected');
                        }
                    } else {
                ...
                switch (type) {
                    case 'dropdown':
                        ...
                        for (var key in returnData.list) {
                            ...
                            if (typeof returnData.hide !== 'undefined') {
                                for (var key2 in returnData.hide) {
                                    if (returnData.list[key] === returnData.hide[key2]) {
                                        available = false;
                                        break;
                                    }
                                }
                    ...
                    case 'list':
                        target.find('.chart-search-list-column').hide();
                        ...
                        target.find('.list-group-item').each(function () {
                            ...
                            if (available && !hide) {
                                $(this).parents('.chart-search-list-column').show();
...
function downloadPDF (chartId, downloadMulti) {
    var data = {};
    downloadMulti = (typeof downloadMulti !== 'undefined' ? downloadMulti : false);

    if (downloadMulti) {
        var group = $('.chart-item-data[data-chart-id=' + chartId + ']').parents('.chart-group');
        group.find('.chart-item-data').each(function () {
            data[$(this).data('chart-id')] = $(this).data();
        });
    } else {
        var chartData = $('.chart-item-data[data-chart-id=' + chartId + ']'),
            group = chartData.parents('.chart-group');
    ...
    var url = '/trade/downloadPdf/';
    if (typeof group.data('chart-type') !== 'undefined') {
        url = '/' + group.data('chart-type') + '/downloadPdf/';
    }
...
function tableFixedHeader () {
    return false;
    $('.chart-item-table table.table').each(function () {
        var $table = $(this);

        //reset
        $table.find('thead').css({display: 'table-header-group'});
        $table.find('tbody').css({display: 'table-row-group'});
        $table.find('.table-freeze').each(function () {
            $(this).addClass('table-freeze-item');
            $(this).html('<div class="table-freeze-head-inner">' + $(this).text() + '</div>');
        });


        $table.parents('.table-responsive-freeze').css({
            position: 'relative',
            'overflow-x': 'visible',
        });
        $table.parents('.table-responsive-inner').css({
            'overflow-x': 'auto',
        });

        //get position when static
        var width = [];
        var position = [];
        var padding = 0;
        $table.find('.table-freeze').each(function (i) {
            padding += $(this).outerWidth();
            width[i] = {
                height: $(this).height(),
                width: $(this).width(),
            };
        ...
        $table.find('.table-freeze').each(function (i) {
            if (typeof position[i] !== 'undefined') {
                console.log('padding', position[i].left, padding, position[i].left - padding);
                position[i].left = position[i].left;
                $(this).css(position[i]);

                $(this).find('.table-freeze-head-inner').css(width[i]);
            }
        });

        $table.find('tbody tr').each(function (i) {
            var bgHeight = bgTop = 0
            $(this).find('th, td').each(function (j) {
                if (typeof width[j] !== 'undefined') {
                    $(this).addClass('table-freeze-item')

                    var positionTemp = position[j];
                    positionTemp.top = $(this).position().top;
                    $(this).css(positionTemp);

                    var widthTemp = width[j];
                    widthTemp.height = $(this).outerHeight();
                    $(this).css(widthTemp);

                    $(this).css('background-color', $(this).parents('tr').css('background-color'));

                    bgHeight = widthTemp.height;
                    bgTop = positionTemp.top;
            ...
            $(elm).css({
                ...
                'border-top': $(this).find('td').css('border-top'),
                'z-index': -1
            });
            $table.before(elm);
        });
        ...
        $table.parents('.table-responsive-inner').css({'margin-left': padding});
        $table.find('.table-freeze-item').css({position: 'absolute'});
    });
}